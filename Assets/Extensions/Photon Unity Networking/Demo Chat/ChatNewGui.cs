...
public class ChatNewGui : MonoBehaviour, IChatClientListener
{
    ...
    public static Dictionary<string,string> TextFriends = new Dictionary<string, string>();
    ...
    public Text InGameText; // set in inspector
    ...
    float delay_ingame_text_time = 0f;

    private readonly Dictionary<string, Toggle> channelToggles = new Dictionary<string, Toggle>();
	
	public readonly Dictionary<string,scr_UIFriend> friendListItemLUT =  new Dictionary<string, scr_UIFriend>();
	...
	public void Start()
	{
        ...
        Destiny.text = CurrentTarget;

        StateText.text  = "";

        TextFriends.Add("@General", "");
        TextFriends.Add("@Matchs", "");
    ...
    {
        if (FindObjectOfType<scr_MNGame>() != null)
        {
            InGameText.gameObject.SetActive(!InGameText.gameObject.activeSelf);
            if (InGameText.gameObject.activeSelf)
                delay_ingame_text_time = 5f;
        } else
        {
            if (ListFriends.IsLoadingDataFriends && Chat.activeSelf)
                return;
    ...
    {
        if (TextFriends.ContainsKey(destiny))
        {
            CurrentTarget = destiny;
            Destiny.text = destiny;
            CurrentChannelText.text = TextFriends[destiny];
        }
    }
	...
	public bool Connect()
	{
        ...
        okr = chatClient.Connect(ChatSettings.Instance.AppId, "1.0", new ExitGames.Client.Photon.Chat.AuthenticationValues(scr_StatsPlayer.Name));
		
		ChannelToggleToInstantiate.gameObject.SetActive(false);
		Debug.Log("Connecting as: " + scr_StatsPlayer.Name);
        return okr;
	...
	public void Update()
	{
        ...
        if (delay_ingame_text_time>0f)
        {
            delay_ingame_text_time -= Time.deltaTime;
            if (delay_ingame_text_time <= 0f)
	...
	public void SendMsg()
	{
        if (InputFieldChat.text!="")
        {
            ...
            {   
                chatClient.PublishMessage(CurrentTarget, InputFieldChat.text, false);
            } else
            {
                chatClient.SendPrivateMessage(CurrentTarget, InputFieldChat.text, false);
                if (TextFriends.ContainsKey(CurrentTarget))
                {
                    TextFriends[CurrentTarget] += "\n" + scr_StatsPlayer.Name + ": " + InputFieldChat.text;
                    CurrentChannelText.text = TextFriends[CurrentTarget];
                }
            }
	...
	public void OnConnected()
	{
        ...
        if (scr_StatsPlayer.Friends!=null  && scr_StatsPlayer.Friends.Count>0)
		{
			chatClient.AddFriends(scr_StatsPlayer.Friends.ToArray()); // Add some users to the server-list to get their status updates
    ...
    void AddTextToUI(string sender, string msg)
    {
        if (TextFriends.ContainsKey(sender))
        {
            TextFriends[sender] += "\n" + sender + ": " + msg;
            if (CurrentTarget == sender)
                CurrentChannelText.text = TextFriends[sender];
        }
    }
	...
	public void OnGetMessages(string channelName, string[] senders, object[] messages)
	{
        ...
        {
            if (TextFriends.ContainsKey("@General"))
            {
                TextFriends["@General"] += "\n" + senders[i].ToString() + ": " + messages[i].ToString();
                if (CurrentTarget == "@General")
                    CurrentChannelText.text = TextFriends["@General"];
            }
        }
	...
	public void OnPrivateMessage(string sender, object message, string channelName)
	{
        ...
        if (TextFriends.ContainsKey(sender))
        {
            ...
            if (msg=="/InviteToPlay")
            {
                Debug.Log("IgotAInvitationFrom: " + sender);
                scr_UIPlayerEditor uip = FindObjectOfType<scr_UIPlayerEditor>();
                if (uip != null)
                {
                    uip.MRequests.Add(sender);
                }

            } else if (msg == "/RefInvitation")
            {
                scr_UIPlayerEditor uip = FindObjectOfType<scr_UIPlayerEditor>();
                if (uip != null)
                {
	...
	public void OnStatusUpdate(string user, int status, bool gotMessage, object message)
	{
		...
		{
			scr_UIFriend _friendItem = friendListItemLUT[user];
	...
	*/
    ...
    {
        scr_StatsPlayer.CurrentState = state;
        chatClient.SetOnlineStatus(state);
    }
	...
	public void ShowChannel(string channelName)
	{
        ...
        /*
        string msgs = channel.ToStringMessages();
